package com.eventbooking.bookingmanagementfinal.Model;
...
public class BookingManagement {
    ...
    /**
     * Scan Booking.txt for the highest N### ID and return the next one (N001, N002…).
     */
    public String getNextBookingID() {
        ...
        try {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (parts.length > 0 && parts[0].startsWith("N")) {
                    try {
                        int idNum = Integer.parseInt(parts[0].substring(1)); // remove 'N' and parse number
                        if (idNum > maxId) {
                            maxId = idNum;
                        }
        ...
        return String.format("N%03d", maxId + 1);  // e.g., N001, N002
    }
